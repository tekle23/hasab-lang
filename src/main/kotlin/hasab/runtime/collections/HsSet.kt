...
/**
 * Immutable set wrapper for the HASAB runtime.
 *
 * Wraps a [Set] of nullable [Any] values and exposes a comprehensive
 * set of read-only operations. Use [HsMutableSet] for mutation.
 */
public class HsSet internal constructor(
    private val backing: Set<Any?>,
) {
     ...
     */
    public fun union(other: HsSet): HsSet = HsSet(backing.union(other.backing))

    /**
     * Returns a new [HsSet] containing only elements present in both this set and [other] (set intersection).
     */
    public fun intersect(other: HsSet): HsSet = HsSet(backing.intersect(other.backing))

    /**
     * Returns a new [HsSet] containing elements in this set but not in [other] (set difference).
     */
    public fun minus(other: HsSet): HsSet = HsSet(backing.minus(other.backing))
    ...
    /**
     * The underlying JVM set. Read-only access for interop.
     */
    public val javaSet: Set<Any?> get() = backing

    override fun equals(other: Any?): Boolean {
        if (this === other) return true
        if (other !is HsSet) return false
        return backing == other.backing
    }

    override fun hashCode(): Int = backing.hashCode()

    override fun toString(): String = "HsSet($backing)"

    public companion object {

        /**
         * Creates an [HsSet] from the given [elements].
         */
        public fun of(vararg elements: Any?): HsSet = HsSet(elements.toSet())

        /**
         * Creates an empty [HsSet].
         */
        public fun empty(): HsSet = HsSet(emptySet())
    }
}
